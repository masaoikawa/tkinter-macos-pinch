...
本プロジェクトは、特定の環境（macOS 26.4 / 25E246）で動作を確認していますが、OSのバージョンやデバイスによって挙動が異なる可能性があります。

動作した・しないに関わらず、[Issues](https://github.com/masaoikawa/tkinter-macos-pinch/issues) にて動作報告をいただけると大変助かります！報告の際は、以下の情報を添えていただけるとスムーズです。

- **macOSのバージョン**
- **Pythonのバージョン**
- **使用したデバイス**（トラックパッド、Magic Mouse、サードパーティ製マウスなど）
- **具体的な挙動**（例：ズームが速すぎる、特定の操作でエラーが出る、期待通りに動いた、など）

皆様からのフィードバックが、このプロジェクトをより安定したものにするための助けになります。
